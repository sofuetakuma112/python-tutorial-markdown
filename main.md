...
Python では, 文章や人名といった文字からなるデータを, **文字列(string)** という.
...
`while`文は, `条件式`が満たされ, 結果が`True`(真)である間, `処理`を繰り返して実行する.
...
`for`文は, `リストオブジェクト`から要素を一つずつ順番に取り出し, それぞれの要素ごとに, `for`文に続けて記述した`処理`を, 一度ずつ実行する. `変数名`には, `リストオブジェクト`から取り出した要素を参照する変数名を指定する.
